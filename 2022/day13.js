...
/** @typedef { { item: number | Item[] }} Item */
/** @typedef { { left: Item, right: Item }} ItemPair */
/** @typedef {true | false | "dunno"} Troolean */
 ...
 * @returns {Troolean}
 */
function leftIsLessThanRight(left, right) {
    /** @type {Item[]} */
    let compareLeft;
    /** @type {Item[]} */
    let compareRight;
    if (isNumber(left)) {
        compareLeft = [{ item: left.item }];
    } else {
        compareLeft = left.item;
    }
    if (isNumber(right)) {
        compareRight = [{ item: right.item }];
    } else {
        compareRight = right.item;
    }
    let i = 0;
    do {
        const l = compareLeft[i];
        const r = compareRight[i];
        if (l == null && r != null) return true;
        if (r == null && l != null) return false;
        if (r == null && l == null) return "dunno";
        if (!isNumber(l) || !isNumber(r)) {
            log("Comparing not Numbers " + l.item + " " + r.item);
            const res = leftIsLessThanRight(l, r);
            if (res == true) return true;
            else if (res == false) return false; 
        } else {
            log("Comparing numbers " + l.item + " " + r.item);
            if (l.item < r.item) return true;
            else if (l.item > r.item) return false;
        }
        i++;
    } while (compareLeft[i] != null || compareRight[i] != null)
    return "dunno";
}
...
function isNumber(item) {
    return typeof item.item == "number"
}
...
/** @type {Item} */
const l = { item: [{ item: 2 }, { item: 3 }, {item: 4}, {item: [ { item: 1 }]}] };

/** @type {Item} */
const r = { item: [{ item: 2 }, { item: 3 }, {item: 4}, {item:2}] };

/** @type {ItemPair[]} */
const itemPairs = [];

/** @type {string} str */
function parseStringToPairs(str) {
    let sum = 0;
    ...
    pairs.forEach((pair, index) => {
        ...
        const l = arrayToItems(JSON.parse(bits[0]));
        const r = arrayToItems(JSON.parse(bits[1]));
        items.push(l);
        items.push(r);
        //console.log(index, l, r);
        const res = leftIsLessThanRight(l, r);
        if (res == true) sum += index + 1;
    });
    console.log(sum);

    const findOne = { item: [{ item: 2 }] };
    const findTwo = { item: [{ item: 6 }] };
    items.push(findOne);
    items.push(findTwo);

    items.sort((a, b) => leftIsLessThanRight(a, b) ? -1 : 1);
    const ii = items.findIndex(a => a == findOne);
    const iii = items.findIndex(a => a == findTwo);
    console.log(ii + 1, iii + 1, (ii + 1) * (iii + 1));
}
...
function arrayToItems(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (typeof arr[i] == "number") {
            item.item.push({ item: arr[i] });
        } else {
            item.item.push(arrayToItems(arr[i]));
        }
    }
    return item;
}

//console.log(leftIsLessThanRight(l, r));

const input = `[[[],2,9,4],[],[[[],[0]],[[7,0,10,8],[10,5,5,4,1],0,6]],[]]

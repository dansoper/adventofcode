const f = function(xx) {
  const xs = xx.split(",").map(a => {
    const direction = a.substring(0,1);
    const amount = parseInt(a.substring(1));
    let x = 0;
    let y = 0;
    if (direction == "U") {
      y = -amount;
...
const h = function(p1, p2) {
  const intersections = [];
  p1.forEach(a => {
    if (p2.find(b => a.x == b.x && a.y == b.y) != null) {
